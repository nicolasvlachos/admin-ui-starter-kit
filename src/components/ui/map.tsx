...
const useTheme = () => ({ resolvedTheme: typeof document !== 'undefined' && document.documentElement.classList.contains('dark') ? 'dark' : 'light' });
import React, {
    Suspense,
    createContext,
    lazy,
...
function MapMarker({
    icon = <MapPinIcon className="size-6 text-foreground" />,
    iconAnchor = [12, 12],
    bgPos,
    popupAnchor,
    tooltipAnchor,
    ...
    return (
        <LeafletMarker
            icon={L.divIcon({
                // Empty className stops leaflet's default `.leaflet-div-icon`
                // (white background + grey border rectangle) from being applied
                // around our SVG marker.
                className: "",
...
function MapLocateControl({
    ...
    const trackingHandler = hasLocation ? stopLocating : startLocating
    const trackingTitle = isLocating
        ? "Locating..."
        : hasLocation
          ? "Stop tracking"
          : "Track location"
    const trackingAriaLabel = isLocating
        ? "Locating..."
        : hasLocation
...
function useLeaflet() {
    ...
    const [LeafletDraw, setLeafletDraw] = useState<LeafletDrawModule | null>(
        null
    )
