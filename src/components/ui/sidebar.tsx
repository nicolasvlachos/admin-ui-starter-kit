...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("gap-2 px-2 flex flex-col", className)}
      {...props}
    />
  )
}
...
function SidebarMenuButton({
  ...
  // When tooltip is enabled AND a custom render element is provided (e.g. <Link>),
  // compose them: render as TooltipTrigger but forward the custom render element
  // so the trigger renders as the Link instead of a plain <button>.
  const tooltipRender = tooltip
    ? render
  ...
  const comp = useRender({
    ...
    render: tooltip ? tooltipRender : render,
    state: {
